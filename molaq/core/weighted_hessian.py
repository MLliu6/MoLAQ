"""
molaq/core/weighted_hessian.py
§3.5: 创新点 A — 模态感知加权 Hessian + GPTQ 逐列量化

核心改变：用模态加权 Hessian H^MoLAQ 替换 GPTQ 的等权 H，
Cholesky 分解与逐列更新循环完全复用 GPTQ。
"""

import torch
from torch import Tensor
from typing import Tuple, TYPE_CHECKING

if TYPE_CHECKING:
    from molaq.stats.modal_stats import LayerStats

from molaq.core.smooth import apply_smooth


def compute_modal_hessian(
    X_tilde:    Tensor,
    alpha_lang: float,
    alpha_sal:  float,
    alpha_bg:   float,
    lang_mask:  Tensor,
    sal_mask:   Tensor,
    bg_mask:    Tensor,
    damp:       float = 1e-3,
) -> Tensor:
    """
    计算 MoLAQ 模态加权 Hessian。

    Args:
        X_tilde    : [N, d_in]  平滑后激活（Stage 3 输入）
        alpha_lang/sal/bg : 各组 Hessian 权重系数（来自 modal_stats，sum=3）
        lang/sal/bg_mask  : [N] bool，token 分组掩码
        damp       : 正则化强度 λ，默认 1e-3
    Returns:
        H : [d_in, d_in]  正定 Hessian 矩阵

    公式（§3.5）：
        H^MoLAQ = (2/3) * (α_lang * G_lang + α_sal * G_sal + α_bg * G_bg)
        H       ← H + λ · tr(H) · I
    """
    d_in = X_tilde.shape[1]  # 取列数标量

    def gram(mask: Tensor) -> Tensor:
        """归一化 Gram 矩阵 G_m = X_m^T X_m / N_m"""
        if mask.sum() == 0:
            return torch.zeros(d_in, d_in, dtype=torch.float32,
                               device=X_tilde.device)
        x_m = X_tilde[mask].float()
        N_m = x_m.shape[0]           # 取行数标量
        return x_m.T @ x_m / N_m    # [d_in, d_in]

    G_lang = gram(lang_mask)
    G_sal  = gram(sal_mask)
    G_bg   = gram(bg_mask)

    H = (2.0 / 3.0) * (
        alpha_lang * G_lang +
        alpha_sal  * G_sal  +
        alpha_bg   * G_bg
    )  # [d_in, d_in]

    # 正则化：H ← H + λ · tr(H) · I（与论文 §3.5 完全对应）
    trace_H = H.trace().item()
    H = H + damp * trace_H * torch.eye(
        d_in, dtype=H.dtype, device=H.device
    )
    return H


def gptq_quantize(
    W_tilde:    Tensor,
    H:          Tensor,
    bits:       int  = 4,
    group_size: int  = 128,
    sym:        bool = True,
) -> Tensor:
    """
    GPTQ Cholesky 逐列量化。

    Args:
        W_tilde    : [d_out, d_in]  待量化权重（平滑后）
        H          : [d_in, d_in]   MoLAQ Hessian
        bits       : 量化位数（默认 4）
        group_size : 量化分组大小（默认 128）
        sym        : 是否对称量化（默认 True）
    Returns:
        W_q : [d_out, d_in]  量化后权重（仍在平滑空间）
    """
    d_out, d_in = W_tilde.shape
    W_q = W_tilde.clone().float()

    try:
        L = torch.linalg.cholesky(H)
    except torch.linalg.LinAlgError:
        # 正定性恢复：补充对角正则化
        d = H.shape[0]  # 取标量维度
        H = H + 1e-2 * H.trace() / d * torch.eye(
            d, dtype=H.dtype, device=H.device
        )
        L = torch.linalg.cholesky(H)

    H_inv = torch.cholesky_inverse(L)  # [d_in, d_in]

    for i in range(d_in):
        w_col   = W_q[:, i]                  # [d_out]
        g_start = (i // group_size) * group_size
        g_end   = min(g_start + group_size, d_in)
        w_group = W_q[:, g_start:g_end]      # [d_out, group_size]

        scale = (
            w_group.abs().max(dim=1, keepdim=True).values
            / (2 ** (bits - 1) - 1)
        )
        scale = scale.clamp(min=1e-8)        # [d_out, 1]

        if sym:
            w_col_q = (
                (w_col / scale.squeeze(1)).round()
                .clamp(-(2 ** (bits - 1)), 2 ** (bits - 1) - 1)
                * scale.squeeze(1)
            )                                # [d_out]
        else:
            raise NotImplementedError("非对称量化暂未实现")

        quant_err = w_col_q - w_col          # [d_out]
        h_inv_row = H_inv[i, i + 1:]        # [d_in - i - 1]
        h_inv_ii  = H_inv[i, i].clamp(min=1e-8)

        W_q[:, i]     = w_col_q
        W_q[:, i + 1:] -= torch.outer(quant_err, h_inv_row / h_inv_ii)

    return W_q


def quantize_layer_A(
    stats,          # LayerStats
    W:          Tensor,
    s:          Tensor,
    enable_C:   bool = True,
    bits:       int  = 4,
    group_size: int  = 128,
) -> Tuple[Tensor, Tensor]:
    """
    Stage 3 完整量化接口。

    Args:
        stats    : LayerStats（来自 modal_stats）
        W        : [d_out, d_in]  原始浮点权重
        s        : [d_in]         smooth 缩放因子（enable_C=False 时传入全 1 向量）
        enable_C : 是否启用预平滑
        bits / group_size : 量化参数
    Returns:
        W_hat_A : [d_out, d_in]  量化后权重（恢复到原始空间）
        s       : [d_in]         实际使用的缩放因子（供后续 B 模块参考）
    """
    if enable_C:
        X_tilde, W_tilde = apply_smooth(stats.X_raw, W, s)
    else:
        X_tilde = stats.X_raw.float()
        W_tilde = W.float()
        # identity 缩放向量：d_in 维全一向量（device 对齐）
        s = torch.ones(W.shape[1], dtype=torch.float32, device=W.device)

    H = compute_modal_hessian(
        X_tilde,
        stats.alpha_lang, stats.alpha_sal, stats.alpha_bg,
        stats.lang_mask, stats.sal_mask, stats.bg_mask,
    )

    W_tilde_q = gptq_quantize(W_tilde, H, bits=bits, group_size=group_size)

    # 恢复到原始空间：Ŵ^(A) = diag(s)^{-1} · Ŵ_tilde
    W_hat_A = W_tilde_q / s.unsqueeze(0)   # [d_out, d_in]

    return W_hat_A, s


def sanity_check_layer(W_hat_A: Tensor, W: Tensor, H: Tensor, X_raw: Tensor):
    """
    量化后必须运行的 sanity check。
    打印量化误差和条件数改善情况。
    超出范围时打印警告（不自动停止，由调用方决定是否 raise）。
    """
    err = (W_hat_A - W.float()).abs()
    mean_err = err.mean().item()
    max_err  = err.max().item()
    print(f"  [sanity] quant error: mean={mean_err:.4f}, max={max_err:.4f}", end="")
    if mean_err > 0.02:
        print(f"  ⚠️  mean > 0.02，检查 damp 正则化强度", end="")
    if max_err > 0.5:
        print(f"  ⚠️  max > 0.5，检查该层权重分布", end="")
    print()

    N = X_raw.shape[0]  # 取行数标量
    H_gptq = (2.0 / N) * X_raw.float().T @ X_raw.float()
    kappa_gptq  = torch.linalg.cond(H_gptq.float()).item()
    kappa_molaq = torch.linalg.cond(H.float()).item()
    print(f"  [sanity] κ(H_GPTQ)={kappa_gptq:.1f}, κ(H_MoLAQ)={kappa_molaq:.1f}", end="")
    if kappa_molaq > kappa_gptq:
        print("  ⚠️  κ(H_MoLAQ) > κ(H_GPTQ)，检查 smooth_scale 是否正确施加", end="")
    print()
