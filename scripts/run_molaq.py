...
设备策略（锥形设计）：
  - forward pass / 激活收集：GPU（device_map=auto）
  - 量化计算（Hessian / GPTQ / smooth / knapsack）：全部在 args.device（cuda）
  - LayerStats 中所有 Tensor：cuda float32，不做 .cpu()
  - 写回权重：.to(orig_device, orig_dtype)
"""
...
def main():
    ...
    for layer_name, module in linear_layers:
        ...
        stats = all_stats[layer_name]  # LayerStats: 所有 Tensor 在 cuda
        ...
        if args.enable_C or args.enable_A:
            smooth_s = (
                compute_smooth_scale(stats.x_bar, W)   # 均在 cuda
                if args.enable_C
        ...
        if args.enable_B:
            ...
            W_final   = saliency_awq_quantize(
                W_input_B, stats.X_raw, stats.x_bar,   # 均在 cuda
        ...
        # 释放当层 stats 显存（196 层累积大）
        del all_stats[layer_name]
